...
etError SHT3x_ReadSerialNumber(u32t* serialNumber)
{
  ...
  command[0] = CMD_READ_SERIALNBR >> 8;
  command[1] = CMD_READ_SERIALNBR & 0x00FF;
  status = eusci_b_i2c_tx_multi(_i2cAddress, command, 2);
  // error = SHT3X_StartWriteAccess();
  
  // // write "read serial number" command
  // error |= SHT3X_WriteCommand(CMD_READ_SERIALNBR);
  
  status = eusci_b_i2c_rx_multi(_i2cAddress, serialNum, 6);
  if(status == 0) {  // tx ack success, receive serial number
    for(int i=0;i<6;i+=3) { 
      txData[0] = *(serialNum + i);
      txData[1] = *(serialNum + (i+1));
      checksum = *(serialNum + (i+2));
      crc = SHT3X_CalcCrc(txData, 2);
      if(crc == checksum) {
        serialNumWords[i/3] = (txData[0] << 8) + txData[1];
      }
      else {
        //crc error
...
//        sgpc10_resistance[i/3] = 0xffff; // max value indicate crc error
  ...
  if(error == NO_ERROR)
  {
    *serialNumber = (((UINT32) serialNumWords[0]) << 16) | serialNumWords[1];
  }
                                    ...
                                    u8t timeout)
{
  ...
  {
    case REPEATAB_LOW:
      command[0] = CMD_MEAS_POLLING_L >> 8;
      command[1] = CMD_MEAS_POLLING_L & 0x00FF;
//        error = SHT3X_WriteCommand(CMD_MEAS_POLLING_L);
      break;
    case REPEATAB_MEDIUM:
      command[0] = CMD_MEAS_POLLING_M >> 8;
      command[1] = CMD_MEAS_POLLING_M & 0x00FF;
//        error = SHT3X_WriteCommand(CMD_MEAS_POLLING_M);
      break;
    case REPEATAB_HIGH:
      command[0] = CMD_MEAS_POLLING_H >> 8;
      command[1] = CMD_MEAS_POLLING_H & 0x00FF;
//        error = SHT3X_WriteCommand(CMD_MEAS_POLLING_H);
  ...
  if(error == NO_ERROR)
  {
    ...
    status = eusci_b_i2c_rx_multi(_i2cAddress, rhtData, 6);
  }
  ...
  crc = SHT3X_CalcCrc(txData, 2);
  if(crc == checksum) {
    rawValueTemp = (txData[0] << 8) + txData[1];
  }
  else {
    error = CHECKSUM_ERROR;
  }
  ...
  crc = SHT3X_CalcCrc(txData, 2);
  if(crc == checksum) {
    rawValueHumi = (txData[0] << 8) + txData[1];
  }
  else {
    error = CHECKSUM_ERROR;
  }
...
etError SHT3X_SoftReset(void)
{
  ...
  if(error == NO_ERROR) delay_micro_second(50000); 
...
//
//  // wait 100 ms
//  delay_micro_second(100000);
//  
//  // release reset
//  RESET_HIGH();
//  
//  // wait 50 ms after reset
//  delay_micro_second(50000);
//}
...
//  error = sht3x_i2c_WriteByte(_i2cAddress << 1);
//
//  return error;
}
...
//  error = sht3x_i2c_WriteByte(_i2cAddress << 1 | 0x01);
//
//  return error;
}
...
//  error  = sht3x_i2c_WriteByte(command >> 8);
//
//  // write the lower 8 bits of the command to the sensor
//  error |= sht3x_i2c_WriteByte(command & 0xFF);
//
//  return error;
}
...
//                        error = sht3x_i2c_ReadByte(&bytes[0], ACK, timeout);
//  if(error == NO_ERROR) error = sht3x_i2c_ReadByte(&bytes[1], ACK, 0);
//  if(error == NO_ERROR) error = sht3x_i2c_ReadByte(&checksum, finaleAckNack, 0);
//  
//  // verify checksum
...
//                        error = sht3x_i2c_WriteByte(bytes[0]); // write data MSB
//  if(error == NO_ERROR) error = sht3x_i2c_WriteByte(bytes[1]); // write data LSB
//  if(error == NO_ERROR) error = sht3x_i2c_WriteByte(checksum); // write checksum
//  
//  return error;
}
